export function HermesLogo() {
  return (
    <svg
      // width="24"
      // height="24"
      viewBox="0 0 20 20"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className='w-12 h-12'
    >
      <path d="M9 6V8L19 3V1L9 6Z" stroke="#111827" strokeWidth="0.7" />
      <path d="M9 8V10L17 6V4L9 8Z" stroke="#111827" strokeWidth="0.7" />
      <path d="M9 10V12L15 9V7L9 10Z" stroke="#111827" strokeWidth="0.7" />
      <path
        d="M9 6L5.5 7.5V13M5.5 15.5C6.86683 14.9142 7.63316 14.5858 9 14V12M5.5 15.5V13M5.5 15.5L1 17.5L2 14.5L5.5 13"
        stroke="#111827"
        strokeWidth="0.7"
      />
      <path
        d="M5.44816 9.58809L6.88191 8.9215M5.61796 11.0785L7.05172 10.4119"
        stroke="#111827"
        strokeWidth="0.7"
      />
    </svg>
  );
}
